...
func TheSpigot(numberOfDigitsToCalc int, done chan bool, webPrint func(string)) {
	...
	SpigotCalculation(numberOfDigitsToCalc, done)

}

var piWithInsertedDecimalPoint []string

func SpigotCalculation(numberOfDigits int, done chan bool) { // Rick's version does not return a string // called by the previous func
	start := time.Now()
