...
func ArchimedesBig(webPrint func(string)) {
	...
	// 2. Give it a slightly longer "Network Breath"
	time.Sleep(50 * time.Millisecond)

	// 3. START WITH TEXT, NO LEADING NEWLINES
	// If the browser sees "data: ", it thinks the message is over.
	webPrint("You've selected a demonstration of Dick's improved version of Archimedes' method for aproximating the value of Pi : 3.14159...")
	 // I am reminded of why I used the feyn desktop version previously. Browsers have their own ideas, and you just have to deal. 
	...
	for i := 0; i < 5001; i++ {
		...
		if i == 24 {
			// webPrint(fmt.Sprintf("%v vs the value of π from the web", p_d)) // This was what was dumping a huge pile of Pie into the browser!!
